...
[`schmidt_quasi_normalized_legendre!`](@ref). The algorithm accesses the terms
`P[n, m + 1]` when the order `m` is lower than the degree `n`. Hence, the matrix `P` must
have at least `n_max + 1` rows and `m_max + 2` columns if `m_max < n_max`, or `m_max + 1`
columns otherwise. This function throws an `ArgumentError` if `P` does not have the
required dimensions.
